...
if [[ $(id -u) -ne 0 ]]; then # From ChatGPT
    echo "This script requires root privileges. Please run with sudo."
    exit 1
fi

if ! command -v apt &> /dev/null; then # From ChatGPT
...
if command -v nvcc &> /dev/null || [[ -d "/usr/local/cuda" ]]; then # From ChatGpt
